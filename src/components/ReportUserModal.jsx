...
export default function ReportUserModal({ isOpen, onClose, userName = 'User' }) {
  ...
  // The modal component itself stays mounted while `isOpen` toggles, so
  // local state (reason/description) previously survived a Cancel and
  // reappeared as a stale draft the next time it opened. Resetting on
  // every open — regardless of how the previous session was closed
  // (Cancel, X, Escape, overlay click) — fixes that.
  useEffect(() => {
    if (isOpen) {
      setReason('');
      setDescription('');
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      setSubmitting(false);
      onClose();
      toast.success(`Report submitted. Our team will review ${userName}'s account shortly.`);
    }, 800);
  };
    ...
    <Dialog isOpen={isOpen} onClose={onClose} title="Report User" size="md">
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <Button variant="danger" onClick={handleSubmit} loading={submitting} disabled={!reason}>
            Submit Report
          </Button>
        </div>
      </div>
